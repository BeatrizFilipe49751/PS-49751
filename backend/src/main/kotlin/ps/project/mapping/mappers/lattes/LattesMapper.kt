...
class LattesMapper(
            ...
            for (entry in entries) {
                ...
                educations.add(
                    EducationDTO(
                        ...
                        startDate = startDate,
                        endDate = endDate,
                        thesis = extractThesis(entry, endDate)
                    )
                )
            }
        }
    ...
    private fun extractThesis(entry: LattesEducation, date: LocalDate): ThesisDTO? {
        ...
        return thesisTitle?.let {
            ThesisDTO(title = it, date = date, supervisors = supervisors)
        }
    }
